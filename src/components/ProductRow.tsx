import { KeepaProduct, getScoreClass, Marketplace, MARKETPLACE_CONFIG, cleanTitle, isGermanTitle } from '@/lib/types';
...
export default function ProductRow({ product, marketplace, onAnalyze }: ProductRowProps) {
  ...
  const german = isGermanTitle(product.title);
    ...
    <div className="glass-card p-4 flex flex-col sm:flex-row items-start sm:items-center gap-4 hover:shadow-md transition-shadow">
      <div className="flex-1 min-w-0">
        <h3 className="font-display font-semibold text-sm truncate">
          {title}
          {german && <span className="ml-1.5 text-xs">🇩🇪</span>}
        </h3>
      ...
      <div className="flex flex-wrap items-center gap-3 text-sm">
        <div className="text-center px-3">
          ...
          <div className="font-semibold">{product.currentPrice != null ? `${currency}${product.currentPrice.toFixed(2)}` : 'N/D'}</div>
        </div>
        <div className="text-center px-3">
          <div className="text-xs text-muted-foreground">{t('product.bsr')}</div>
          <div className="font-semibold">{product.bsr != null ? product.bsr.toLocaleString() : 'N/D'}</div>
        </div>
        <div className="text-center px-3">
          <div className="text-xs text-muted-foreground">{t('product.reviews')}</div>
          <div className="font-semibold">{product.reviewCount ?? 'N/D'}</div>
        </div>
        <div className="text-center px-3">
